...
static void list_insert_before(addr_list_t **head, addr_list_t **tail,
                               addr_list_t *before_this, addr_list_t *item) {
  if (before_this) {
...
static int list_push_sorted(addr_list_t **head, addr_list_t **tail,
                            addr_list_t *item) {
  addr_list_t *p = *head;
  int rc;
  while (p) {
...
int list_copy(addr_list_t *head, addr_list_t *tail, addr_list_t **thead,
              addr_list_t **ttail) {
  addr_list_t *p0 = head;
  addr_list_t *item;
  while (p0) {
...
static void addr_print_v4(FILE *o, addr_t *addr) {
  fprintf(o, "%hhu.%hhu.%hhu.%hhu", addr->addr >> 24, (addr->addr >> 16) & 0xff,
...
static inline size_t addr_v4_weight(int cidr) { return 1ULL << (32 - cidr); }

static size_t addr_start_col = 1, addr_start_row = 1, parse_col = 1,
              parse_row = 1;
        ...
        *p = '\0';
        if (!addr_parse_v4(b, p - b, &(item->addr))) {
          ...
          if (list_push_sorted(head, tail, item)) {
            ...
            // function detected copy of this address. Skip
          }
        }
        p = b;
      }
...
int main(int argc, const char **argv) {
  ...
  if (!args.no_stats) {
    printf("coverage=%ld, source=%ld, falsely_covered=%lf%%; result=%d, compress=%lf%%\n",
           compress_stats.coverage, compress_stats.source_count,
           100.00f - ((double)compress_stats.source_count / compress_stats.coverage *
               100.00f),
           count, 100.00f - ((double)count / compress_stats.source_count * 100.00f));
  }
      ...
      if (ftell(o) > 0) {
        if (strcmp(args.input, "-") == 0) {
          if (args.overwrite) {
            ...
            fprintf(stderr, "Output file is not empty. Cancelled.\n");
